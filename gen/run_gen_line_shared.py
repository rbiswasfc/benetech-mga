...
from src.line_plot_shared import SharedLinePlot
from src.line_xy_generation import generate_from_shared
...
def generate_plot_data(cfg, generator):
    data = next(generator)
...
def main(args, cfg):
    generator = generate_from_shared()
    ...
    for _ in range(cfg.num_images):
        ...
        the_example = generate_plot_data(cfg, generator)
        ...
        try:
            SharedLinePlot(cfg, the_example, texture_files=texture_files).make_line_plot(image_id)
