    ...
    for example in wiki_bank:
        ...
        if (len(cat_series_list) > 0) & (len(num_series_list) > 0):
            ...
            anno = {
                ...
                # edit these for horizontal plots ---
                'y_title': xs['series-name'],
                'x_title': ys['series-name'],
...
def generate_integers(n_points, uniform=True, length_scale=25, **kwargs):
    ...
    if p <= 0.1:
        ...
        start = random.randint(0, length_scale*length_scale)
    else:
        start = random.randint(-length_scale*length_scale, length_scale*length_scale)
...
def generate_floats(n_points, uniform=True, length_scale=25.0, **kwargs):
    ...
    if p <= 0.1:
        ...
        start = random.uniform(0, length_scale*length_scale)
    else:
        start = random.uniform(-length_scale*length_scale, length_scale*length_scale)
...
def generate_x(stem_glossary):
    ...
    func = random.choices(func_list, weights=[0.6, 0.1, 0.1, 0.1, 0.1], k=1)[0]
