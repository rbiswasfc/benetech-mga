...
from src.line_plot_advanced import BasicLinePlot
...
def generate_plot_data(cfg, wiki_generator, synthetic_generator):
   ...
   # max data points in a plot
    ...
    x_pre = []
    x_post = []
    if (len(x_series) >= 6) & (random.random() >= 0.1):
        # ---
        N = len(x_series)
        n_pre = 1
        n_post = 1

        if (random.random() >= 0.8) & (N >= 10):
            n_pre = random.randint(2, 3)

        if (random.random() >= 0.8) & (N >= 10):
            n_post = random.randint(2, 3)
